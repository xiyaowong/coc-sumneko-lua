...
if os.getenv('NEOVIM_SRC'):
    base_dir = os.getenv('NEOVIM_SRC') or os.path.dirname(os.path.dirname(script_path))
else:
    base_dir = input('input neovim src dir: ') or os.path.dirname(
        os.path.dirname(script_path)
    )
base_dir = os.path.expanduser(os.path.expandvars(base_dir))
# for filter cmd
os.environ['NEOVIM_SRC'] = base_dir
...
mpack_output_dir = 'nvim_lua_data'
if not os.path.exists(mpack_output_dir):
    os.mkdir(mpack_output_dir)
...
CONFIG = {
    ...
    'lua': {
        ...
        'helptag_fmt': lambda name: (
            '*lua-vim*' if name.lower() == '_editor' else f'*lua-{name.lower()}*'
        ),
        'fn_helptag_fmt': lambda fstem, name: (
            f'*vim.{name}()*' if fstem.lower() == '_editor' else f'*{fstem}.{name}()*'
        ),
        'module_override': {
    ...
    # add filetype, highlight, keymap, ui
    'filetype': {
        'mode': 'lua',
        ...
        'files': ['runtime/lua/vim/filetype.lua'],
        'file_patterns': '*.lua',
        'fn_name_prefix': '',
        'section_name': {'diagnostic.lua': 'diagnostic'},
        'section_fmt': lambda _: 'Lua module: vim.diagnostic',
    ...
    'highlight': {
        'mode': 'lua',
        'filename': 'highlight.txt',
        'section_order': [
            'highlight.lua',
        ],
        'files': ['runtime/lua/vim/highlight.lua'],
        'file_patterns': '*.lua',
        'fn_name_prefix': '',
        'section_name': {'diagnostic.lua': 'diagnostic'},
        'section_fmt': lambda _: 'Lua module: vim.diagnostic',
    ...
    'keymap': {
        ...
        'files': ['runtime/lua/vim/keymap.lua'],
        'file_patterns': '*.lua',
        'fn_name_prefix': '',
        'section_name': {'diagnostic.lua': 'diagnostic'},
        'section_fmt': lambda _: 'Lua module: vim.diagnostic',
...
def render_node(n, text, prefix='', indent='', width=62):
    ...
    if n.nodeName == 'preformatted':
        ...
        for c in n.childNodes:
            if is_inline(c) and '' != get_text(c).strip() and text and ' ' != text[-1]:
                text += ' '
        ...
        for c in n.childNodes:
            ...
            text += '{}\n'.format(
                render_node(
                    c, text, prefix='{}. '.format(i), indent=indent, width=width
                )
            )
            i = i + 1
...
def main(config, args):
    ...
    for target in CONFIG:
        ...
        mpack_file = os.path.join(
            mpack_output_dir, CONFIG[target]['filename'].replace('.txt', '.mpack')
        )
        if os.path.exists(mpack_file):
            os.remove(mpack_file)

        output_dir = out_dir.format(target=target)
        log.info("Generating documentation for %s in folder %s", target, output_dir)
        ...
        print(
            ' '.join(
                [
                    ...
                    for file in CONFIG[target]['files']
                ]
            ),
        )
        p.communicate(
        ...
        for compound in dom.getElementsByTagName('compound'):
            ...
            if filename.endswith('.c') or filename.endswith('.lua'):
                xmlfile = os.path.join(
                    base, '{}.xml'.format(compound.getAttribute('refid'))
                )
...
def parse_args():
    ...
    ap = argparse.ArgumentParser(description="Generate helpdoc from source code")
    ap.add_argument(
        "--log-level",
        "-l",
    ...
    ap.add_argument('source_filter', nargs='*', help="Filter source file(s)")
    ap.add_argument(
        '-k', '--keep-tmpfiles', action='store_true', help="Keep temporary files"
    )
    ap.add_argument('-t', '--target', help=f'One of ({targets}), defaults to "all"')
