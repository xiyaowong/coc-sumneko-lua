...
local data_dir = cwd .. "/nvim_lua_data/"
local types_dir = cwd .. "/nvim_lua_types/"
...
function M.parse(mpack, prefix, exclude)
	...
	local writer = M.writer(types_dir .. fname)
	local classes = {}
...
function M.options()
	local data = vim.fn.json_decode(vim.fn.readfile(data_dir .. "builtin-docs.json"))
	...
	local writer = M.writer(types_dir .. "options")
...
function M.functions()
	...
	local writer = M.writer(types_dir .. "vim.fn")
...
function M.clean()
	for _, f in pairs(vim.fn.expand(types_dir .. "*.lua", false, true)) do
		if f ~= types_dir .. "vim.lua" then
			uv.fs_unlink(f)
		end
	end
end
...
function M.build()
	...
	M.parse("filetype.mpack", "vim.filetype")
	M.parse("highlight.mpack", "vim.highlight")
	M.parse("keymap.mpack", "vim.keymap")
	M.parse("ui.mpack", "vim.ui")
